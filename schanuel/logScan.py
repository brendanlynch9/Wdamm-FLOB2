import numpy as np
from scipy.linalg import solve
import time

def solve_marchenko_schanuel(z_set, L=25.0, N=600):
    """
    High-resolution solver for SIR sensitivity analysis.
    """
    x_vals = np.linspace(0.01, L, N)
    dx = x_vals[1] - x_vals[0]
    
    elements = []
    for z in z_set:
        elements.append(z)
        elements.append(np.exp(z))
    elements = np.array(elements, dtype=complex)
    
    kappas = 0.12 + 0.015 * np.log(1 + np.abs(elements))
    alphas = elements / (1 + np.abs(elements)) 

    def B_func(t):
        exponent = -kappas[:, None, None] * t[None, :, :]
        return np.real(np.sum(alphas[:, None, None] * np.exp(exponent), axis=0))

    K_xx = np.zeros(N)
    for i in range(N):
        sub_grid = x_vals[i:]
        n_sub = len(sub_grid)
        if n_sub < 2: break
        
        x = x_vals[i]
        B_mat = B_func(sub_grid[:, None] + sub_grid[None, :]) * dx
        A = np.eye(n_sub) + B_mat
        rhs_val = -np.real(np.sum(alphas[:, None] * np.exp(-kappas[:, None] * (x + sub_grid)[None, :]), axis=0))
        
        try:
            sol = solve(A, rhs_val, assume_a='pos')
            K_xx[i] = sol[0]
        except:
            K_xx[i] = 0.0

    V = -2 * np.gradient(K_xx, dx)
    mask = ~np.isnan(V)
    l1 = np.trapz(np.abs(V[mask]), dx=dx)
    l2 = np.sqrt(np.trapz(V[mask]**2, dx=dx))
    sir = l2 / l1 if l1 > 0 else 0
    return l1, sir

# --- Collision Precision Scan ---
print(f"{'Epsilon (Separation)':<20} | {'L1 Mass':<15} | {'SIR':<15}")
print("-" * 55)

epsilons = [1e-3, 1e-6, 1e-9, 1e-12]
base_val = 1.0

for eps in epsilons:
    # Testing a triplet collision around base_val
    z_set = [base_val, base_val + eps, base_val + 2*eps]
    l1, sir = solve_marchenko_schanuel(z_set)
    print(f"{eps:<20} | {l1:<15.6f} | {sir:<15.6f}")

# Final Baseline Comparison
l1_ind, sir_ind = solve_marchenko_schanuel([1.0, np.e, np.pi])
print("-" * 55)
print(f"{'Indep [1, e, π]':<20} | {l1_ind:<15.6f} | {sir_ind:<15.6f}")

# (base) brendanlynch@Brendans-Laptop schanuel % python logScan.py
# Epsilon (Separation) | L1 Mass         | SIR            
# -------------------------------------------------------
# 0.001                | 0.505732        | 0.237160       
# 1e-06                | 0.505687        | 0.237160       
# 1e-09                | 0.505687        | 0.237160       
# 1e-12                | 0.505687        | 0.237160       
# -------------------------------------------------------
# Indep [1, e, π]      | 0.571074        | 0.235618       
# (base) brendanlynch@Brendans-Laptop schanuel % 