...
def evaluate_schanuel_uft_f(z_set, name, L=30.0, N=800):
    ...
    # 2. Trigger ACI via Overload Amplification (Additive Relations)
    for i in range(n):
        for j in range(i + 1, n):
            for k in range(n):
                if k != i and k != j:
                    res = np.abs(z_array[i] + z_array[j] - z_array[k])
                    if res < 1e-8: # Tighter tolerance for high-precision constants
...
# --- Schanuel Unknowns Test ---
cases = [
    ("Baseline [1, e, pi]", [1.0, np.e, np.pi]),
    ("Sum Check [e, pi, e+pi]", [np.e, np.pi, np.e + np.pi]),
    ("Product Check [e, pi, e*pi]", [np.e, np.pi, np.e * np.pi]),
    ("Mixed Set [e, pi, e+pi, e*pi]", [np.e, np.pi, np.e + np.pi, np.e * np.pi])
]
...
#     (base) brendanlynch@Brendans-Laptop schanuel % python Epi.py
# ---------------------------------------------------------------------------
# Case                           | SRC        | NTD        | MaxCond   
# ---------------------------------------------------------------------------
# Baseline [1, e, pi]            | 1.00       | 0.87       | 3.29e+01  
# Sum Check [e, pi, e+pi]        | 125.80     | 224.80     | 1.16e+12  
# Product Check [e, pi, e*pi]    | 1.23       | 1.56       | 2.99e+01  
# Mixed Set [e, pi, e+pi, e*pi]  | 125.79     | 224.78     | 1.16e+12  
