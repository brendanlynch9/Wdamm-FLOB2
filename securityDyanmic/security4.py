...
secure_precheck_with_seed.py

Same secure, low-latency payload authentication pre-check demo as before,
with an added state pre-seed in the main block so the demo will accept the
first valid time-step packet on its initial run.
...
- Demo flow showing accept, replay reject, out-of-window reject, and rate-limiter behavior
- Pre-seed helper to initialize device state to current_time_counter() - 1
"""
...
def log(msg: str) -> None:
    ...
    try:
        ...
        # Logging failure should not crash demo
        pass
...
class StateManager:
    ...
    Includes a 'seed_state' helper to pre-seed the state for demo/sync purposes.
    """
    ...
    def seed_state(self, last_accepted_counter: int) -> None:
        """
        Pre-seed the persistent state. Useful in demos or when initializing
        devices after an out-of-band synchronization step.
        """
        s = {
            "last_accepted_counter": int(last_accepted_counter),
            "fail_count": 0,
            "backoff_until": 0.0
        }
        self._write_state(s)
        log(f"Pre-seeded state: last_accepted_counter = {last_accepted_counter}")
    ...
    def record_failure(self) -> None:
        ...
        # exponential backoff (starts only after RATE_LIMIT_MAX_FAILS)
...
def verify_tag(key: bytes,
    ...
    # Verify tag for that received counter
...
def demo_flow(state_mgr: StateManager, key_bytes: bytes):
    log("=== Demo start ===")
    ...
    # Receiver: attempt to verify (expected success after pre-seed)
    ...
    # We'll send tampered_payload with original tag (mismatch) to simulate tampering
...
# -------------------------
# Main: ensure dirs exist, optionally pre-seed state, run demo
# -------------------------
if __name__ == "__main__":
    ...
    # -------------------------
    # DEMO FIX: Pre-seed state for an initial successful verification
    # -------------------------
    # This simulates an initial out-of-band synchronization where the receiver's
    # stored counter is set to one less than the sender's current time counter.
    try:
        current_c = current_time_counter()
        state_mgr.seed_state(current_c - 1)
    except Exception as e:
        log(f"Warning: Could not pre-seed state file for demo: {e}")

    # Run the demo
    demo_flow(state_mgr, key_bytes)
